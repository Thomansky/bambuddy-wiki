...
!!! info "Explicit starts are not held"
    Both holds apply to the queue's *automatic* dispatch only. **Print now** in the print dialog, the :material-play: on a staged queue item and the actions on a printer card start regardless of a pending run or a slot ahead &mdash; the run then waits for that print, as it would for any print on the printer.
