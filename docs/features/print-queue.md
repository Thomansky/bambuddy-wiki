...
- **A slot nothing can read is tried once.** Some spools have no tag at all &mdash; a third-party roll, a torn or unreadable label. If a read leaves the slot still unidentified, Bambuddy remembers that slot and stops asking about it, so it does not cost every following job an attempt. The memory is meant for that one spool and is dropped again the next time Bambuddy sees the slot empty or the AMS names what is in it (it looks once per queue check, so a spool swapped in and out between two checks can go unnoticed). It also never silences a slot the printer itself reports as not yet read &mdash; which is what a spool put in during a print looks like &mdash; so a fresh Bambu spool in that slot is always offered its read.
